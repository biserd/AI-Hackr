...
- **Background worker** (`server/background-worker.ts`):
  ...
  - **Manual scan limits (accepted product decision)**: Pro 3 per rolling 24h, Free 1 per rolling 7d. The free-tier allowance is intentional — it gives free users a taste of the manual-rescan UX (a key Pro upsell hook) without flooding the worker. If product later wants strict Pro-only manual scans, gate `manualScanNow` in `server/background-worker.ts` and the scan-now button in `client/src/pages/watchlist.tsx`.
